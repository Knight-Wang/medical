...
class sim_computation:
    ...
    def sim_words_tfidf_all(self, s, t,icd6):
        sw = self.getWords(s)
        tw = self.getWords(t)
    ...
    def sim_words_tfidf(self, s, t, icd6, tfidf_dict):
            ...
            if s_segs_tfidf.sum() == 0: # 均匹配不上
                ...
                sim_tfidf = 1 - self.cosineSimilarity(s_segs_tfidf, t_vector)  # cosine similarity

            sim_w = float(len(sw & tw)) / float(max(len(sw), len(tw)))
            sim = max(sim_tfidf, sim_w)
            return sim

    def cosineSimilarity(self, s, t):
        res = 0.0
        length = len(s)
        sumx = 0.0
        sumy = 0.0
        ...
        if sumx != 0 and sumy != 0:
            return res/math.sqrt(sumx * sumy)
        else:
            return 0.0