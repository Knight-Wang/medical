    ...
    for i in range(len(res)):
        if len(res[i]) < 3:
            if len(result) == 0:
                ...
                result[len(result) - 1] = result[len(result) - 1] + res[i]
        else:
            result.append(res[i])
    if tmp != "" :
        if len(result) > 1:
...
def sim_mention_entity(mention, e, icd6, tfidf_dict):
    ...
    sim_w = sc.sim_words_tfidf_all(mention, e, icd6)  # 字集合和tfidf的相似度
...
def getMappingResult_segs(name_segs, normalized_dic, tfidf_dict, icd3names): #return name, flag(compute_brother_nodes)
    seg_sim = []
    ...
    # check icd3 name
    father_match = {}
    for (icd3, name) in icd3names.items():
            icd3 = icd3[:3]
            sim_fathernode = sc.sim_words(name, name_all_str)
            father_match[icd3] = True if sim_fathernode > 0.1 else False
    ...
    for i in range(length): # 诊断的每个片段的匹配
        ...
        if len(location) != 0:  # 存在部位
            ...
            for disease_name in normalized_dic.keys():
                ...
                icd3 = icd6.split(".")[0]
                if icd3 in father_match.keys():
                    if father_match[icd3] == False:
                        continue
